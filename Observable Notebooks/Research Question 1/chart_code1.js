...
function _d3(require){return(
  ...
  // ——— Helper: Encode Categorical Variables as Integers ———
function _encodeCategory(){return(
  ...
  // ——— Helper: Rescale Correlation Values for Visualization ———
...
// ——— Chart 1: Factor-to-Disease Correlation Heatmap ———
function _chart1(pointBiserialCorrelation, data, d3) {
  // Set chart dimensions and margins
  const chartWidth = 800;
  const chartHeight = 700;
  const margin = { top: 120, right: 150, bottom: 120, left: 100 };

  // Define target diseases and influencing factors
  ...
  // Calculate correlations between each factor and each disease
  ...
  // Scales for positioning the cells
  const xScale = d3.scaleBand().domain(targets).range([margin.left, chartWidth - margin.right]).padding(0.02);
  const yScale = d3.scaleBand().domain(factors).range([margin.top, chartHeight - margin.bottom]).padding(0.15);

  // Color scale (Red = positive correlation, Blue = negative)
  ...
  // Draw rectangles for each correlation cell
  svg.selectAll('rect')
  ...
  // Add numeric correlation labels inside each cell
  svg.selectAll('text')
    .data(correlationMatrix)
    .enter()
    .append('text')
  ...
  // Add color legend for correlation scale
  ...
  gradient.append('stop').attr('offset', '0%').attr('stop-color', "#61b8e7"); // blue
  gradient.append('stop').attr('offset', '100%').attr('stop-color', "#d23626"); // red
  ...
  legend.append('text')
    .attr('x', 0).attr('y', legendHeight + 15)
    .attr('text-anchor', 'middle')
    .style('font-family', 'Arial, sans-serif')
    .style('font-size', '12px')
  ...
  legend.append('text')
    .attr('x', legendWidth).attr('y', legendHeight + 15)
    .attr('text-anchor', 'middle')
    .style('font-family', 'Arial, sans-serif')
    .style('font-size', '12px')
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("chart1")).define("chart1", ["pointBiserialCorrelation","data","d3"], _chart1);
